...
export class TwitterApiService {
  ...
  getOrPost(method, url, token, tokenSecret, bodyParams): Promise<any> {
    ...
    let arrQueryParams = TwitterApiService.getQueryParamsFromUrl(url);
    let oauth_consumer_key = CONSUMER_KEY;
    let oauth_nonce = TwitterApiService.getNonce(32);
    ...
    let parameterString = TwitterApiService.getSignatureString(arrQueryParams.sort(function (a, b): number {
    ...
    let authValue = TwitterApiService.getAuthValue([
      {key: "oauth_consumer_key", value: oauth_consumer_key},
      {key: "oauth_nonce", value: oauth_nonce},
    ...
    if (method == 'GET') {
      ...
      return this.http.get(url, new RequestOptions({headers: headers})).toPromise()
        ...
        .catch(TwitterApiService.handleError);
    }
    else {
  ...
  onSignIn(): Promise<any> {
    ...
    let twitterCallbackUrl = 'https://example.com:4200/callback';
    ...
    let oauth_timestamp = Math.floor(Date.now() / 1000).toString();
    let oauth_version = "1.0";
    let parameterString = TwitterApiService.getSignatureString([
  ...
  getSuggestedUsers() {
    ...
    this.getOrPost(method, url, this.userInfo["oauth_token"], this.userInfo["oauth_token_secret"], [])
      .then(response => {
        console.log("This response i am not able to see: ", response);
        let slug = TwitterApiService.randomElement(JSON.parse(response["_body"]))["slug"];
